(function ($) {
  ...
  var plusApp = {
    ...
    initChosen: function (element) {
      $(element, plus).chosen();
      $(element, plus).chosen({default_no_result_text: 'asdasd'});
    },
    ...
    hashChange: function () {
      ...
      if (plusApp.init === true) return false;
      var hash_views = {
        '#null': '.init_view',
        '#etusivu': '.init_view',
    ...
    initEvents: function () {
      ...
      plus.on('change', '.municipality_selection', function (event) {
        if ($(this).find('option:selected').val() !== '') {
          ...
          $('.homecareunit_selection', plus).trigger('chosen:updated');
          /*jshint -W030 */
          ...
          $('.municipality_selection', plus).trigger('chosen:updated');
          $('.selected_municipality', plus).hide().fadeIn(500).html(plusApp.selectedMunicipality);
          $('.answer', plus).hide().fadeIn(500);

          plusApp.updateColumnChart($('.municipality_selection', plus).find('option:selected').data('value-2013'), $('.municipality_selection', plus).find('option:selected').data('value-2030'));
